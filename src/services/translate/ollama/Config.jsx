...
import { createOllamaTranslationPrompt, upgradeOllamaTranslationPrompt } from './prompt';
...
export function Config(props) {
    ...
    async function pullModel() {
        setIsPulling(true);
        const ollama = new Ollama({ host: serviceConfig.requestPath });
        // 旧服务配置页即使被外部深链打开，也只能安装当前统一模型。
        const stream = await ollama.pull({ model: UNIFIED_OLLAMA_MODEL, stream: true });
    ...
    useEffect(() => {
        ...
        const promptList = upgradeOllamaTranslationPrompt(serviceConfig.promptList);
        if (promptList !== serviceConfig.promptList) {
            // 精确匹配旧内置模板时自动迁移；自定义提示词不会被覆盖。
            setServiceConfig({ ...serviceConfig, promptList }, true);
        }
    }, [serviceConfig]);
        ...
        serviceConfig !== null && (
            <form
                onSubmit={(e) => {
                    e.preventDefault();
                    setIsLoading(true);
                    translate('hello', Language.en, Language.zh_cn, {

import { Card, CardBody, CardFooter, Button, Chip, Skeleton, ButtonGroup, Tooltip } from '@nextui-org/react';
...
export default function TextArea(props) {
    ...
    useEffect(() => {
        ...
        if (
            ...
            setLoading(true);
            let preserveStructure =
                getServiceSouceType(currentServiceInstanceKey) !== ServiceSourceType.PLUGIN &&
                builtinServices[getServiceName(currentServiceInstanceKey)]?.info?.structuredOutput === true;
                        ...
                        .recognize(
                            ...
                            builtinServices[getServiceName(currentServiceInstanceKey)].Language[language],
                            {
                                ...
                                onResultMetadata: (metadata) => {
                                    if (recognizeId === id) {
                                        preserveStructure = metadata.structured === true;
    ...
    return (
        ...
        >
            ...
            <CardFooter className='bg-content1 flex justify-between px-[12px]'>
                <ButtonGroup>
                ...
                {resultMetadata?.source === 'system' && (
                    <Tooltip
                        content={resultMetadata.fallbackDetail || t('services.recognize.ollama_ocr.fallback_notice')}
                    >
                        <Chip
                            size='sm'
                            color='warning'
                            variant='flat'
                        >
                            {t('services.recognize.ollama_ocr.fallback_badge')}
                        </Chip>
                    </Tooltip>
                )}
            </CardFooter>

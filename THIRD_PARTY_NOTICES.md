...
小允翻译使用了多个开源项目。以下是主要运行时组件的简要索引；精确版本以
`pnpm-lock.yaml` 与 `src-tauri/Cargo.lock` 为准，各项目的许可证及版权声明
仍归原作者所有。

| 组件                                                      | 用途                       | 主要许可证          |
| --------------------------------------------------------- | -------------------------- | ------------------- |
| [Pot Desktop](https://github.com/pot-app/pot-desktop)     | 上游翻译应用基础           | GPL-3.0             |
| [Tauri](https://github.com/tauri-apps/tauri)              | Windows 桌面运行时         | Apache-2.0 / MIT    |
...
| [selection](https://crates.io/crates/selection)           | Windows 选区取词基础       | GPL-3.0-only        |

## 外部软件与模型

-   [Ollama](https://ollama.com/) 由用户独立安装，不随小允翻译分发。
-   `gemma4:e4b-it-qat` 由 Ollama 模型库按需下载，不随小允翻译分发，并受
    Google Gemma 独立使用条款约束。
